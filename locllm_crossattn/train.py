...
MAX_LEN = 64 + len(tokenizer.encode(prompt).ids)
...
def encode_pair(text_a: str, text_b: str) -> np.ndarray:
    ...
    # Combine all parts
    encoded = system_prompt + user_input + assistant_prompt + sep_id + assistant_response
...
# Build and load VLM
vlm = VLM(cfg, activate_crossattn=True).to(device)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if local_rank == 0:
        save_path = "vlm_trained_weights.pt"
