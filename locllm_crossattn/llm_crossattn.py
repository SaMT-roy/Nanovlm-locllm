...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask=None) -> Tuple[torch.Tensor, dict]:
        """
...
class MultiHeadCrossAttention(nn.Module):
    """
    Vanilla multi-head scaled dot-product attention (no masking).
    ...
    def __init__(self, num_heads, d_model, dropout=0.0):
        ...
        # Apply standard initialization first (Xavier/Kaiming)
        self._init_standard()
        
        # OVERWRITE the output projection to be Zero for slowly introducing image context as training progresses.
        nn.init.zeros_(self.wo.weight)
        if self.wo.bias is not None:
            nn.init.zeros_(self.wo.bias)

    def _init_standard(self):
        # Initialize Wq, Wk, Wv with normal distribution
        nn.init.xavier_normal_(self.wq.weight)
        nn.init.xavier_normal_(self.wk.weight)
        nn.init.xavier_normal_(self.wv.weight)
        
        # Initialize Wo normally first (though we overwrite it immediately after)
...
class CrossAttention(nn.Module):
    """
    Q = text tokens
    K,V = image tokens (already projected)
    """
    def __init__(self, num_heads, d_model, dropout=0.0):
        super().__init__()
        self.norm_x = RMSNorm(cfg)
        self.norm_kv = RMSNorm(cfg)
        self.mha = MultiHeadCrossAttention(num_heads, d_model, dropout)
    ...
    def forward(self, x, kv):
        ...
        attn_out = self.mha(query=x_norm, key=kv_norm, value=kv_norm)
        return x + attn_out

    
class LanguageModelBlock(nn.Module):
    def __init__(self, cfg, activate_crossattn=False):
        super().__init__()
        ...
        self.activate_crossattn = activate_crossattn
        if activate_crossattn:
            self.cross_attn = CrossAttention(num_heads=cfg.lm_n_heads, d_model=cfg.lm_hidden_dim)
    
    def forward(self, x: torch.Tensor, kv_img: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask: torch.Tensor=None):
        """
        ...
        if self.activate_crossattn and kv_img is not None:
            x = self.cross_attn(x, kv_img)
...
class LanguageModel(nn.Module):
    def __init__(self, cfg, activate_crossattn=False):
        ...
        # • If activate_crossattn == False → no block uses cross-attn
        # • If activate_crossattn == True → only every nth block uses cross-attn
        self.blocks = nn.ModuleList()
        for i in range(cfg.lm_n_blocks):
            use_crossattn = activate_crossattn and (i % 5 == 0)
            self.blocks.append(
                LanguageModelBlock(cfg, activate_crossattn=use_crossattn)
            )
    ...
    def forward(self, x: torch.Tensor, kv_img: torch.Tensor=None, attention_mask: torch.Tensor=None):
        """
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, kv_img, cos, sin, attention_mask)
    ...
    def generate(self, inputs: torch.Tensor, kv_img: torch.tensor=None, max_new_tokens: int=20):
        """
        ...
        if kv_img is not None and kv_img.dim()==1:
            kv_img = kv_img.unsqueeze(0)

        generated_outputs = inputs.clone()

        for i in range(max_new_tokens):
            generated_emb = self.token_embedding(generated_outputs)
            prompt_output = self.forward(generated_emb, kv_img, attention_mask=None)
    ...
    @classmethod
    def from_pretrained(cls, cfg, activate_crossattn=False):
        ...
        model = cls(cfg, activate_crossattn=activate_crossattn)
