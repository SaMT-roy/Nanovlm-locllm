...
tokenizer = Tokenizer.from_file("/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/mpii/smollm/tokenizer.json")
...
class VLM(torch.nn.Module):
    def __init__(self, cfg, activate_crossattn=False):
        super().__init__()
        # --- Language Model ---
        self.decoder = LanguageModel.from_pretrained(cfg, activate_crossattn=activate_crossattn)
    ...
    def forward(self, inputs):
        ...
        logits = self.decoder(text_emb, img_emb, attention_mask=pad_mask)
        return logits
    ...
    def generate(self, inputs, max_new_tokens=30, temp=1.0):
        ...
        generated_outputs = text_emb
        newly_generated_ids_list = []

        for i in range(max_new_tokens):
            # Forward pass
            prompt_output = self.decoder(generated_outputs, img_emb, attention_mask=None)
...
# Build and load VLM
vlm = VLM(cfg, activate_crossattn=True)
state_dict = torch.load(
    '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_cross_attention/vlm_trained_weights.pt',
    map_location=device
)
...
def encode(text: str) -> List[int]:
    ...
    full_ids = system_prompt + user_input + assistant_prompt
    return full_ids
...
def generate_answer_cocoval(model, image_path, annotations_path, question, kpt_name, save_dir, max_tokens=30, temp=0.5):
    ...
    for i in range(len(images)):
        ...
        base_name = os.path.basename(image_path)       
        name_no_ext = os.path.splitext(base_name)[0]   
        filename = f"{name_no_ext}_{i}_{kpt_name}.png"
        filepath = os.path.join(save_dir, filename)
...
save_dir = "model_inference_epoch5/img0"
...
save_dir = "model_inference_epoch5/img1"
...
save_dir = "model_inference_epoch5/img2"
...
save_dir = "model_inference_epoch5/img3"
...
save_dir = "model_inference_epoch5/img4"
    
img = '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_author/coco_val2017/000000022479.jpg'
ann = '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_author/coco_keypoints_val2017.json'
for kpt, desc in KeypointLocationDescription.items():
    ques = f"{desc} Find the coordinates of {kpt} of this person from the image."
    print(f"\n Predicting: {kpt}")
...
save_dir = "model_inference_epoch5/img5"
...
save_dir = "model_inference_epoch5/img6"
    
img = '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_author/coco_val2017/000000021604.jpg'
ann = '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_author/coco_keypoints_val2017.json'
for kpt, desc in KeypointLocationDescription.items():
    ques = f"{desc} Find the coordinates of {kpt} of this person from the image."
    print(f"\n Predicting: {kpt}")
...
save_dir = "model_inference_epoch5/img7"
...
save_dir = "model_inference_epoch5/img8"
