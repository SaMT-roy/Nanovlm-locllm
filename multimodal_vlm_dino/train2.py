...
load_result = vlm.load_state_dict(new_state_dict, strict=False)
...
# Llava Dataset
image_folder = "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_author/coco_train2017"

paths = [
    "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/llava/complex_reasoning_77k.json",
    "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/llava/detail_23k.json",
    "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/llava/llava_instruct_150k.json",
]

data = []
for p in paths:
    with open(p, "r") as f:
        data.extend(json.load(f))   # 🔹 combine all into one list
...
for item in tqdm(data, desc="Processing samples"):
    try:
        ...
        human_msg = conv[0]["value"].replace("<image>", "").strip()
        gpt_msg   = conv[1]["value"].strip()
...
print("\nVALID samples (combined llava):", df1.shape)
print("MISSING images:", len(missing))

del rows, missing, data

# LNQA Dataset
df2 = pd.read_csv('/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/lnqa/train_qa.csv')

print('initial training samples lnqa:', len(df2))
df2 = df2[df2['img_path'].apply(os.path.exists)].reset_index(drop=True)
print("Remaining samples lnqa:", len(df2))
...
print("subsampled 2 lnqa:", len(df2))

# LRV negative dataset
json_path = "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/lrv_instruct/Filter_cap_negative.json"
image_folder = "/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/nanovlm/data/lrv_instruct/negative_lrv_visual_genome_320"
...
print("\nVALID samples lrv:", df3.shape)
print("MISSING images lrv:", len(missing))
...
print("subsampled 2 lrv:", len(df3))
del rows, missing, data
...
for epoch in range(1, EPOCHS + 1):
    ...
    if local_rank == 0:
        save_path = f"vlm_trained_weights_stage2_epoch{epoch}.pt"
