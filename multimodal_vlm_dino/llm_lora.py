...
class LoraLayer(torch.nn.Module):
    def __init__(self, input_dim, output_dim, rank=32, alpha=64, name='lora'):
        super().__init__()
        self.rank = rank
        self.alpha = alpha
        ...
        # LoRA dense layers
        self.A = torch.nn.Linear(in_features=input_dim, out_features=rank, bias=False)
        self.B = torch.nn.Linear(in_features=rank, out_features=output_dim, bias=False)
        
        # Initialize weights
        torch.nn.init.kaiming_normal_(self.A.weight, mode='fan_in', nonlinearity='relu')
        torch.nn.init.zeros_(self.B.weight)

    def forward(self, x):
        return self.B(self.A(x)) * self._scale
...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def __init__(self, cfg, activate_lora=False):
        ...
        if self.activate_lora:
            self.lora_wk = LoraLayer(self.embd_dim, self.head_dim * self.n_kv_heads, name='lora_wk')
            self.lora_wv = LoraLayer(self.embd_dim, self.head_dim * self.n_kv_heads, name='lora_wv')
    ...
    def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask=None, img_weight_mask=None) -> Tuple[torch.Tensor, dict]:
        """
        ...
        if self.activate_lora:
            k_curr = (self.k_proj(x)+self.lora_wk(img_weight_mask*x)).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2) # (B, n_kv_heads, T_curr, head_dim)
            v_curr = (self.v_proj(x)+self.lora_wv(img_weight_mask*x)).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2) # (B, n_kv_heads, T_curr, head_dim)
        else:
        ...
        q, k = apply_rotary_pos_embd(q_curr, k_curr, cos, sin)
        v = v_curr
...
class LanguageModelBlock(nn.Module):
    def __init__(self, cfg, activate_lora=False):
        ...
        self.attn = LanguageModelGroupedQueryAttention(cfg, activate_lora=activate_lora)
        self.norm1 = RMSNorm(cfg) # Input Norm
        self.norm2 = RMSNorm(cfg) # Post Attention Norm

        self.activate_lora = activate_lora
        if self.activate_lora:
            self.lora_injector = LoraLayer(cfg.lm_hidden_dim, cfg.lm_hidden_dim, name='lora_injector')
    
    def forward(self, x: torch.Tensor, img_x: torch.tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask: torch.Tensor=None, img_weight_mask: torch.Tensor=None):
        """
        ...
        res = x
        x = self.norm1(x)
        x = self.attn(x, cos, sin, attention_mask, img_weight_mask)
        x = res + x
        ...
        if self.activate_lora:
            x += self.lora_injector(img_weight_mask * img_x)
...
class LanguageModel(nn.Module):
    def __init__(self, cfg, activate_lora=False):
        ...
        self.blocks = nn.ModuleList([
            LanguageModelBlock(cfg, activate_lora=activate_lora) for _ in range(cfg.lm_n_blocks)
        ])
    ...
    def forward(self, x: torch.Tensor, img_x: torch.Tensor, attention_mask: torch.Tensor=None, img_weight_mask: torch.Tensor=None):
        """
        ...
        current_position_ids = torch.arange(0, T_curr, device=x.device).unsqueeze(0).expand(B, -1)

        cos, sin = self.rotary_embd(current_position_ids) # Get rotary position embeddings for current tokens

        for i, block in enumerate(self.blocks):
            x = block(x, img_x, cos, sin, attention_mask, img_weight_mask)
    ...
    def generate(self, inputs: torch.Tensor, max_new_tokens: int=20, img_weight_mask: torch.Tensor=None):
        """
        ...
        for i in range(max_new_tokens):
            generated_emb = self.token_embedding(generated_outputs)
            prompt_output = self.forward(generated_emb, attention_mask=None, img_weight_mask=img_weight_mask)
            ...
            if i == max_new_tokens - 1 or next_output==2: # "<|im_end|>"
                break

        return generated_outputs

    @torch.inference_mode()
    def batch_generate(self, inputs: torch.Tensor, attention_mask: torch.Tensor, max_new_tokens: int = 20, temp: int = 1):
        """
        Generates tokens with batching and proper attention masking.
        """
        ...
        # Ensure mask aligns with inputs
        if attention_mask is None:
            attention_mask = torch.ones_like(inputs)

        generated_outputs = inputs.clone()
        curr_mask = attention_mask.clone()
        
        eos_id = 2   # <|im_end|>
        
        # Track which sequences in the batch have finished
        # (batch_size,) boolean tensor. False = still generating.
        finished_mask = torch.zeros(inputs.size(0), dtype=torch.bool, device=inputs.device)

        for i in range(max_new_tokens):
            
            generated_emb = self.token_embedding(generated_outputs)
            
            # IMPORTANT: Pass the mask to the model!
            # The mask prevents the model from looking at the <pad> tokens from the prompt
            prompt_output = self.forward(generated_emb, attention_mask=curr_mask)

            last_output = prompt_output[:, -1, :] / temp
            next_output = torch.argmax(last_output, dim=-1, keepdim=True) # (batch, 1)

            # -----------------------------
            # Handle Early Stopping per Sequence
            # -----------------------------
            # Check which sequences just hit EOS
            just_finished = (next_output.squeeze(-1) == eos_id)
            
            # Update the finished state
            finished_mask = finished_mask | just_finished
            
            # If a sequence is already finished, we shouldn't change its output purely for logic's sake,
            # but in simple autoregressive loops, we usually append anyway and strip later.
            # Ideally, we append the EOS token repeatedly if it's finished, so the length stays consistent.
            # Here we just append the output.
            generated_outputs = torch.cat((generated_outputs, next_output), dim=1)
            
            # We must extend the attention mask because the new token is "real"
            # (It is 1, regardless of whether it is EOS or a word)
            new_mask_col = torch.ones((inputs.size(0), 1), dtype=curr_mask.dtype, device=inputs.device)
            curr_mask = torch.cat((curr_mask, new_mask_col), dim=1)

            # Stop if all sequences in the batch are finished
            if i == max_new_tokens - 1 or finished_mask.all():
                break

        return generated_outputs
    
    @classmethod
    def from_pretrained(cls, cfg, activate_lora=False):
        ...
        model = cls(cfg, activate_lora=activate_lora)

        model_dir = '/pfs01/performance-tier/rd_ici/algo_train/saptamtdir/saptamt/locllm_nanovlm/nanovlm3/model_weights'
        ...
        if os.path.exists(index_path):
            ...
            safetensors_files = [os.path.join(model_dir, fn) for fn in safetensors_filenames]
        else:
            safetensors_files = [os.path.join(model_dir, "model.safetensors")]
